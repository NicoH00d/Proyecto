import time
#importar tiempo para que en los ciclos, las funciones no aparezcan de golpe y desorienten al usuario.
...
#crear una lista para después promediar
def promedio(n):
    sum=0
    for x in n:
        sum=sum+x
        prom=sum/len(n)
    return prom
#realiza el producto punto entre vectores(listas)
def propun(vector, vector2):
    suma=0
    for i in range(len(vector)):
        suma=suma+vector[i]*vector2[i]
    return suma
#-----------------inicia--------------------#
print("Hola, Bienvenido")
def valeria ():
    print("\n---C A L C U L A D O R A    G E O M É T R I C A    A R I T M É T I C A---\n¿Qué desea realizar hoy?")
    problema=input(" 1 para problemas geometricos\
\n 2 para problemas aritmeticos\
\n 3 para obtener un promedio\
\n 4 problemas de vectores (beta)\
\n opción: ")
    
    while problema != "1" and problema != "2" and problema !="3" and problema !="4"  :
    ...
    if problema =="1":
        ...
        while arit != "6":
            ...
            time.sleep(1)
            arit=input("\n------------------------------------------------------\nEscoga el numero correspondiente de lo que desee realizar\n1. SUMA\n2. RESTA\n3. Multiplicacion\n4. División.\n5. Elevar a potencia\n6. Salir\n")
#valeria(): volver a llamar a funcion principal 
        valeria()
        ...
        vector=[]
        vector2=[]
        print("Actualmente, solo es posible trabajar con PRODUCTO PUNTO")
        print("utiliza los vectores con valores de i, j, k")
        print("para el vector 1:")
        i=float(input("introduce el valor de i: "))
        vector.append(i)
        j=float(input("introduce el valor de j: "))
        vector.append(j)
        k=float(input("introduce el valor de k: "))
        vector.append(k)
        matriz.append(vector)
        
        print("para el vector 2:")
        ii=float(input("introduce el valor de i: "))
        vector2.append(ii)
        jj=float(input("introduce el valor de j: "))
        vector2.append(jj)
        kk=float(input("introduce el valor de k: "))
        vector2.append(kk)
        matriz.append(vector2)
        print(f"Con tus vectores {matriz}", end=" ")
        print(f"el producto punto de tus vectores es de {propun(vector, vector2)} unidades")
        time.sleep(1)
        print(".")
        time.sleep(1)
        print(".")
